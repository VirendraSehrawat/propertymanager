...
export default function TenantDashboard() {
    ...
    // --- FIXED: Split into primitives to prevent infinite re-renders ---
    ...
    useEffect(() => {
        ...
        // FIXED: Only depend on user.email, not the entire user object!
        return () => { unsubUnit(); unsubVacant(); unsubApps(); unsubTickets(); unsubInvoices(); unsubBuildings(); unsubSettings(); };
    }, [user?.email, role]);
    ...
    const handleAppSubmit = async (e: React.FormEvent) => {
        e.preventDefault(); if (!user?.email || !selectedUnit || !idFile) { alert("ID Proof is required!"); return; } setIsSubmittingApp(true);
        try {
            const idRef = ref(storage, `applications/${user.uid}/id_${Date.now()}_${idFile.name}`); await uploadBytes(idRef, idFile); const idUrl = await getDownloadURL(idRef);
            let payUrl = ""; if (paymentFile) { const payRef = ref(storage, `applications/${user.uid}/payment_${Date.now()}_${paymentFile.name}`); await uploadBytes(payRef, paymentFile); payUrl = await getDownloadURL(payRef); }
            await addDoc(collection(db, "applications"), { tenantEmail: user.email.toLowerCase(), unitId: selectedUnit.id, unitNumber: selectedUnit.unitNumber, buildingId: selectedUnit.buildingId, securityDeposit: selectedUnit.baseRent, transactionId: appTxnId || "", idProofUrl: idUrl, paymentProofUrl: payUrl, status: "pending", createdAt: new Date().toISOString() });
            setIsAppModalOpen(false); setSelectedUnit(null); setIdFile(null); setPaymentFile(null); setAppTxnId("");
        } catch (error) { console.error(error); alert("Upload failed. Please try again."); } finally { setIsSubmittingApp(false); }
    };

    const handleUpdatePayment = async (e: React.FormEvent) => {
        e.preventDefault(); if (!selectedAppToUpdate || !paymentFile || !appTxnId) return; setIsSubmittingApp(true);
        try {
            const payRef = ref(storage, `applications/${user?.uid}/payment_${Date.now()}_${paymentFile.name}`); await uploadBytes(payRef, paymentFile); const payUrl = await getDownloadURL(payRef);
            await updateDoc(doc(db, "applications", selectedAppToUpdate.id), { paymentProofUrl: payUrl, transactionId: appTxnId });
            setIsUpdatePaymentModalOpen(false); setSelectedAppToUpdate(null); setPaymentFile(null); setAppTxnId("");
        } catch (error) { console.error(error); alert("Payment upload failed."); } finally { setIsSubmittingApp(false); }
    };

    const handlePayInvoice = async (e: React.FormEvent) => {
        e.preventDefault(); if (!selectedInvoice || !payTxnId) return; setIsSubmittingPayment(true);
        try { await updateDoc(doc(db, "invoices", selectedInvoice.id), { status: "pending", transactionId: payTxnId, submittedAt: new Date().toISOString() }); setIsPayModalOpen(false); setSelectedInvoice(null); setPayTxnId(""); }
        catch (error) { console.error(error); alert("Failed to submit payment."); } finally { setIsSubmittingPayment(false); }
    };

    const handleMaintenanceSubmit = async (e: React.FormEvent) => {
        e.preventDefault(); if (!user?.email || !unit || !maintDesc) return; setIsSubmittingMaint(true);
        try {
            let photoUrl = ""; if (maintFile) { const fileRef = ref(storage, `maintenance/${user.uid}/${Date.now()}_${maintFile.name}`); await uploadBytes(fileRef, maintFile); photoUrl = await getDownloadURL(fileRef); }
            await addDoc(collection(db, "maintenance"), { tenantEmail: user.email.toLowerCase(), unitId: unit.id, unitNumber: unit.unitNumber, buildingName, category: maintCategory, description: maintDesc, photoUrl, status: "pending", createdAt: new Date().toISOString() });
            setMaintCategory("Plumbing"); setMaintDesc(""); setMaintFile(null); setIsMaintModalOpen(false);
        } catch (error) { alert("Failed to submit ticket."); } finally { setIsSubmittingMaint(false); }
    };
    ...
    const upiLink = selectedInvoice && upiId
        ? `upi://pay?pa=${upiId}&pn=${encodeURIComponent(payeeName)}&am=${selectedInvoice.totalAmount}&cu=INR`
        : "";
    ...
    return (
        <div className="min-h-screen bg-gray-50">
            ...
            <main className="p-6 max-w-4xl mx-auto mt-4">
