...
export default function TenantDashboard() {
    ...
    const handlePayInvoice = async (e: React.FormEvent) => {
        e.preventDefault();
        if (!selectedInvoice || !payTxnId || !payScreenshot) { alert("Please upload payment screenshot and enter transaction ID."); return; }
        setIsSubmittingPayment(true);
        try {
            const amountPaid = payAmount ? Number(payAmount) : Number(selectedInvoice.totalAmount);

            // Upload screenshot
            const screenshotUrl = await uploadFile(`payments/${user!.uid}/${Date.now()}_${payScreenshot.name}`, payScreenshot);
                                            ...
                                            {bldg.availableUnits.map((vUnit: any) => {
                                                ...
                                                return (
                                                    <div key={vUnit.id} className="bg-white border border-gray-200 rounded-lg p-5 flex flex-col justify-between shadow-sm">
                                                        <div><h4 className="font-bold text-lg text-gray-900">{vUnit.unitNumber}</h4><p className="text-gray-600 mt-1">Rent: ₹{vUnit.baseRent || 8000}/mo | Security: ₹{vUnit.baseRent || 8000}</p></div>
                                                        <div className="mt-4">
